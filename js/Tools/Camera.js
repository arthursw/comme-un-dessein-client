...
(function() {
  ...
  define(['paper', 'R', 'Utils/Utils', 'i18next'], function(P, R, Utils, i18next) {
    ...
    Camera = (function() {
      function Camera() {
        this.onSliderDrag = bind(this.onSliderDrag, this);
      }
      ...
      Camera.prototype.onSliderDrag = function(event) {
        if (this.sliders[name].dragging) {
          this.setParameter(name, event);
          event.preventDefault();
          event.stopPropagation();
        }
        return -1;
      ...
      Camera.initializeSliders = function(name) {
        ...
        sliderJ.find('.cd-inline').mousemove(Camera.onSliderDrag);
        sliderJ.find('.cd-inline').on('touchmove', Camera.onSliderDrag);
